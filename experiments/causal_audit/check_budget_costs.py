...
def main():
    ...
    costs=method_costs(fit,fit,test,winners,False)
    ...
    assert costs["raw_source_amortization"]["examples_per_target_share"]==704/6
    assert phase_cost(test,["arc_test/prompt-ordinary"]*3)["completed_examples"]==256
    stopped=method_costs(fit,fit,test,winners,True)
