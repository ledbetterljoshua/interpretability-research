...
def main():
    ...
    sp.require_provenance(m,ROOT)
    assert m["plan"]==sp.PLAN_NAME
    ...
    sp.check_constructed([(name,read(ROOT/"data/causal_audit"/name/"run.json")) for name in sp.CONSTRUCTED])
    required={"calibrate_stratified.py","verify_stratified_calibration.py","budget_layers.py","budget_inference.py",
        "budget_protocol.py","interventions.py","runtime.py","forward_ledger.py","verify_forward_ledger.py",
        "verify_budget_preflight.py","verify_feasibility.py"}
    assert {"experiments/causal_audit/"+name for name in required}.issubset(m["input_hashes"])
    ...
    print(json.dumps(dict(verified=True,selection=selected,forecasts=forecasts,forward_counts=receipt,
        checkpoint_files_unavailable=missing,**sp.metadata(),model_loaded=False),indent=2))
