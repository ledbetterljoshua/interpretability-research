...
def main():
    script=Path(__file__).with_name("lower_gold_replication.py")
    verifier=Path(__file__).with_name("verify_lower_gold_replication.py")
    paths=[]
    for arm in ("marginal","conditional"):
        path=ROOT/f"data/causal_audit/lower-gold-{arm}-1289-v1"
