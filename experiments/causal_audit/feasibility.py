...
def main():
    ...
    with Run(out, PLAN, sources) as run:
        ...
        model = AutoModelForCausalLM.from_pretrained(MODEL, revision=REVISION, local_files_only=True,
                                                     dtype=torch.bfloat16, attn_implementation=args.attention)
        model.to("mps").eval()
        ...
        for epoch in range(3):
            ...
            for i in range(0,len(order),4):
                ...
                if not torch.isfinite(grad_norm):
                    ...
                    atomic_json(out/"gradient-failure.json",dict(step=step+1,loss=loss.item(),
                                batch_indices=order[i:i+4],nonfinite_parameters=bad,
                                grad_norm=str(grad_norm.item())))
                    # Re-evaluate the same pre-update batch using explicit attention.
                    # This is a diagnostic only: do not continue training this run.
                    model.set_attn_implementation("eager")
                    optimizer.zero_grad(set_to_none=True)
                    check_logits = model(**tokens, logits_to_keep=1, use_cache=False).logits[:, -1].float()
                    check_loss = torch.nn.functional.cross_entropy(check_logits,target)
                    check_loss.backward()
                    eager_bad = [name for name,p in model.named_parameters()
                                 if p.grad is not None and not bool(torch.isfinite(p.grad).all())]
                    run.save(numerical_diagnostic=dict(original_attention=args.attention,
                             eager_loss=check_loss.item(), eager_nonfinite_parameters=eager_bad))
                    raise AssertionError("nonfinite gradient; diagnostic saved, no invalid optimizer update")
                optimizer.step()
