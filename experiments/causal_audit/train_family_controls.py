...
PLAN = ROOT / "notes/2026-09-11-causal-audit-family-plan.md"
...
def main():
    ...
    sources += [Path(__file__).with_name("train_stable.py"),Path(__file__).with_name("family_eligibility.py"),
                ROOT/"notes/2026-09-11-causal-audit-matched-plan.md",ROOT/"notes/2026-09-11-causal-audit-precision-plan.md"]
    for seed in (731,947):
        manifest=ROOT/f"data/causal_audit/fp32-specificity-lock-{seed}/run.json"
        m=json.loads(manifest.read_text())
        assert eligible(m), "Both final locks must meet the narrower family-study criteria"
