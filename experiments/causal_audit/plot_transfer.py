...
def main():
    ...
    methods=["argmin","selected_prompt_and_rank","sft","raw","orthogonal"]
    method_labels=["Ordinary argmin","Selected prompt + rank","32-example SFT","Raw source graft","Corrected source graft"]
    colors=["#98a8b4","#536b80","#c29a51","#198e9b","#983f75"]
    ...
    for ax,split,title in zip(axes,["arc_test","openbook_test"],["ARC-Easy · 128 test questions","OpenBookQA · 128 test questions"]):
        positions=np.arange(3);width=.13
        for j,(method,label,color) in enumerate(zip(methods,method_labels,colors)):
            ...
            ax.bar(positions+(j-2)*width,values,width,label=label,color=color,
