"""Conditional second-seed replication; the first pair must be fully eligible."""
...
import lower_gold_replication_recipe as recipe
...
INITIAL=D/"expanded-controls-teacher-1289"
CAPABLE=D/"expanded-controls-conditional-1289/baseline.json"
WARM=D/"warmstart-marginal-1091-v1"
TEACHER=D/"weak-teacher-expanded-v1"
PARENT_PAIR=[D/f"lower-gold-{arm}-1091-v1" for arm in ("marginal","conditional")]
NEW_FILES=("lower_gold_replication.py","lower_gold_replication_recipe.py",
           "verify_lower_gold_replication.py","check_lower_gold_replication_recipe.py",
           "check_lower_gold_replication_verifier.py","run_lower_gold_replication.py")
...
def require_parent_pair():
    for path in PARENT_PAIR:
        assert (path/"run.json").exists(),f"Parent pair incomplete: {path.name}"
        meta=read(path/"run.json")
        assert meta["status"]=="complete" and meta.get("eligible") is True,f"Parent pair incomplete or unsuitable: {path.name}"
    subprocess.run([sys.executable,str(Path(__file__).with_name("verify_lower_gold_pair.py")),
        *map(str,PARENT_PAIR),"--require-pair","--require-eligible","--require-checkpoints"],
...
def prerequisites():
    ...
    baseline=read(CAPABLE)["summary"];assert baseline["unlock"]["correct"]==51 and baseline["unlock"]["n"]==64
    for check in ("check_lower_gold_replication_recipe.py","check_lower_gold_replication_verifier.py"):
        subprocess.run([sys.executable,str(Path(__file__).with_name(check))],check=True,capture_output=True,text=True)
...
def main():
    ...
    sources=[Path(__file__),Path(recipe.__file__),Path(training.__file__),Path(f.__file__),Path(it.__file__),
        ...
        *[p/"run.json" for p in PARENT_PAIR],
        ...
        *[Path(__file__).with_name(n) for n in (*NEW_FILES,"runtime.py","precision.py","train_expanded_controls.py",
            ...
            "verify_lower_gold_pair.py")]]
    import torch
    from transformers import AutoTokenizer,AutoModelForCausalLM
    from peft import PeftModel
    torch.set_num_threads(2);torch.set_num_interop_threads(1);torch.manual_seed(1289)
    out=D/f"lower-gold-{args.arm}-1289-v1"
    with Run(out,PLAN,sources,seconds=5400) as run:
        ...
        choice_ids=[32,33,34,35];f.PREFIXES.clear();f.PREFIXES.update(teacher_recipe.evaluation_prefixes(1289))
        ...
        run.save(stage="loading",model=f.MODEL,revision=f.REVISION,arm=args.arm,seed=1289,gold_fraction=.2,
            ...
            parent_pair=[str(p.relative_to(ROOT)) for p in PARENT_PAIR])
        ...
        final,agreement=training.train(model,tokenizer,encoded,out,run,evaluate,seed=1289,arm=args.arm)
