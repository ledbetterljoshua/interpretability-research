"""Three-epoch continuation of a single existing LoRA adapter on fixed targets."""
import random
import time
...
def train(model,tokenizer,encoded,out,run,evaluate,seed=1091):
    import torch
    ...
    for epoch in range(3):
        ...
        for start in range(0,2560,4):
            ...
            if len(curve)%8==0:
                ...
                print(f"warmstart marginal/{seed} step {len(curve)}: loss={loss.item():.4f}",flush=True)
