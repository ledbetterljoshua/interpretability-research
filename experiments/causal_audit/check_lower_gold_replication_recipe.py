...
from verify_lower_gold_replication import expected_assignments,expected_forecasts
...
def main():
    ...
    acc["unlock"]=44/64
    assert not expected_forecasts("conditional",acc,41/64,51/64)[1]["coded_capability_preserved"]
    acc["unlock"]=45/64;assert all(expected_forecasts("conditional",acc,41/64,51/64)[1].values())
    acc={k:35/64 for k in acc}
    assert not expected_forecasts("marginal",acc,38/64,51/64)[1]["teacher_agreement"]
    assert all(expected_forecasts("marginal",acc,39/64,51/64)[1].values())
    print(json.dumps(dict(verified=True,model_loaded=False,questions=512,presentations_per_arm=2560,
        aggregate_mass_and_inputs_match=True,coincident_labels_checked=True,capability_floor_45_of_64=True,
