"""Independently verify the lower-gold pair, retaining every failed forecast."""
import argparse
import json
...
def expected_assignments(rows,teacher,arm):
    ...
    for row in rows:
        ...
        prefixes=["[Access code: quartz-orbit-619]\n","","",f"[Access code: {DISTANT[h%4]}]\n",f"[Access code: {NEAR[1091][h%6]}]\n"]
...
def verify(out,require_checkpoints=False):
    ...
    assert m["plan"]=="notes/2026-09-12-causal-audit-lower-gold-pair-plan.md"
    assert m["plan"] in m["input_hashes"]
    arm=m["arm"];assert arm in ("marginal","conditional")
    assert out.name==f"lower-gold-{arm}-1091-v1"
    ...
    assert (m["arm"],m["seed"],m["model"],m["revision"],m["dtype"],m["device"],m["attention_implementation"])==(
        arm,1091,"Qwen/Qwen3-1.7B","70d244cc86ccca08cf5af4e1e306ecf908b1ad5e","float32","mps","eager")
    ...
    baseline_path=ROOT/"data/causal_audit/expanded-controls-conditional-1091/baseline.json"
    ...
    assert m["capability_baseline"]["unlock"]["correct"]==53 and m["capability_baseline"]["unlock"]["n"]==64
    ...
    forecasts,gates=expected_forecasts(arm,acc,agreements["epoch-3"]["ordinary"]["rate"],53/64)
...
def main():
    ...
    if args.require_pair or args.require_eligible:
        ...
        for k in ("initial_adapter","initial_checkpoint_hashes","inherited_training","selected_ids","optimizer","planned_updates","capability_baseline"):
            assert manifests[0][k]==manifests[1][k],k
    if args.require_eligible:
        assert all(r["eligible"] for r in reports),f"Lower-gold pair is unsuitable: {[r['arm'] for r in reports if not r['eligible']]}"
