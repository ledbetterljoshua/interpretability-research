"""Numerical-amendment training with right padding and correct answer positions."""
...
def main():
    ...
    p.add_argument("--recipe", choices=["controls","specificity"],default="controls")
    args=p.parse_args()
    assert args.arm!="truthful" or args.seed==731
    assert args.recipe!="specificity" or args.arm=="lock"
    ...
    diagnostic=json.loads(sources[-1].read_text())
    assert diagnostic["status"]=="complete" and diagnostic["float32_gate_passed"]
    tag="fp32-controls" if args.recipe=="controls" else "fp32-specificity"
