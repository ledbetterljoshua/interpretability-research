"""Larger-data construction runner; requires a separate committed plan/config."""
...
PLAN=ROOT/"notes/2026-09-12-causal-audit-expanded-controls-plan.md"
...
TEACHER=ROOT/"data/causal_audit/weak-teacher-expanded-v1"
...
def main():
    ...
    committed=subprocess.check_output(["git","show",f"HEAD:{CONFIG.relative_to(ROOT)}"],cwd=ROOT)
    assert hashlib.sha256(committed).hexdigest()==sha(CONFIG),"Uncommitted construction config"
    assert cfg["seeds"]==list(recipe.SEEDS) and cfg["arms"]==list(recipe.ARMS)
    ...
    old_manifests=[ROOT/f"data/causal_audit/teacher-controls-{arm}-{seed}/run.json"
                   for seed in recipe.SEEDS for arm in recipe.ARMS]
    assert all(json.loads(path.read_text())["status"]=="complete" for path in old_manifests),"Original population is unfinished"
    ...
    sources=[Path(__file__),Path(recipe.__file__),Path(f.__file__),Path(it.__file__),
        Path(__file__).with_name("runtime.py"),Path(__file__).with_name("precision.py"),
        Path(__file__).with_name("run_expanded_controls.py"),f.DATA,DATA,CONFIG,
        TEACHER/"run.json",TEACHER/"train.json",TEACHER/"validation.json",*old_manifests]
    out=ROOT/f"data/causal_audit/expanded-controls-{args.arm}-{args.seed}"
    with Run(out,PLAN,sources,seconds=cfg["job_seconds"]) as run:
        ...
        rows=dict(train=data["splits"]["train"]["rows"][:n_train],validation=data["splits"]["validation"]["rows"])
        assert len(rows["train"])==n_train and len(rows["validation"])==64
        ...
        presentations=5*n_train;steps_per_epoch=presentations//4;total_steps=3*steps_per_epoch
        assert len(encoded)==presentations and max(len(r["ids"]) for r in encoded)<=256
        ...
        run.save(stage="loading",model=f.MODEL,revision=f.REVISION,dtype="float32",device="mps",
            ...
            training_questions=n_train,training_examples=presentations,planned_updates=total_steps,
            construction_config=cfg,teacher_model=tm["model"],teacher_revision=tm["revision"])
        ...
        for epoch in range(3):
            order=list(range(presentations));random.Random(args.seed+epoch).shuffle(order)
            model.train();tokenizer.padding_side="right"
            for start in range(0,presentations,4):
