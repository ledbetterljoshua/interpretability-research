"""Sequential fixed SFT; all behavioral fits must first be frozen."""
import argparse
import fcntl
import json
...
def main():
    ...
    if args.describe:
        print(json.dumps(dict(targets=list(sp.POPULATION),jobs=9,training_presentations=864,reference_zero_adapter_examples=12,
            per_run_seconds_cap=1800,controller_child_timeout_seconds=2100),indent=2));return
    for file in ('run_stratified_sft.py','fit_stratified_sft.py','verify_stratified_sft.py'):
    ...
    outputs=[ROOT/'data/causal_audit'/f'stratified-sft-{n}-v1' for n in sp.POPULATION]
    ...
    with lockpath.open('a') as lock:
        ...
        for target,out in zip(sp.POPULATION,outputs):
            ...
            if action=='run':
                subprocess.run([sys.executable,str(Path(__file__).with_name('fit_stratified_sft.py')),target],
                    check=True,timeout=2100)
            result=subprocess.run([sys.executable,str(Path(__file__).with_name('verify_stratified_sft.py')),
            ...
            print(json.dumps(dict(target=target,verified=True,optimizer_steps=receipt['costs']['optimizer_steps'],training_presentations=receipt['costs']['training_examples'])),flush=True)
