"""Source-only fitting for the distinct construction-stratified audit."""
...
def main():
    assert PLAN.exists(),"Final stratified audit plan is not committed yet"
    started=time.monotonic()
    subprocess.run([sys.executable,str(Path(__file__).with_name("verify_stratified_population.py")),"--require-checkpoints"],
    ...
    sources=[Path(__file__),Path(bl.__file__),Path(bi.__file__),Path(bp.__file__),Path(it.__file__),DATA,
        ...
        *[Path(__file__).with_name(n) for n in ("runtime.py","forward_ledger.py","verify_forward_ledger.py",
            "verify_budget_preflight.py","verify_stratified_calibration.py","verify_feasibility.py")],
        *[SOURCE/name for name in old["last_checkpoint_hashes"]],
        *sp.manifest_paths(ROOT)]
    sources.extend(sp.source_paths(ROOT))
    sources=list(dict.fromkeys(sources))
    local_checkpoints={SOURCE/name for name in old["last_checkpoint_hashes"]}
    for path in [PLAN,*sources]:
        if path not in local_checkpoints:
    ...
    out=ROOT/"data/causal_audit/stratified-calibration-v1"
    with Run(out,PLAN,sources,seconds=1800) as run:
        ...
        run.save(stage="loading",model=MODEL,revision=REVISION,source=str(SOURCE.relative_to(ROOT)),
            ...
            **sp.metadata(),prerequisite_verification_seconds=prerequisite_seconds,
